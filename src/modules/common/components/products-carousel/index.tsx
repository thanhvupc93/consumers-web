...
type ProductsCarouselProps = {
    listsData?: CategoryProductType[];

}
export default function ProductsCarousel({ listsData }: ProductsCarouselProps) {
    const listItems = listsData?.map((item: CategoryProductType) => {
        return (
            <>
                <div className="lg:w-[25%] pb-1 pt-2">
                    <div className="pr-3">
                        <Image className="lg:w-[100%] w-[85%]  rounded-lg" src={item.image} width={350} height={275} alt='Picture of the author ' />
                        <div className="text-left pt-6">
                            <a className=" lg:text-xl  text-sm">{"Grey hoodie"}</a>

                        </div>
                        <div className="text-left mb-2 ">
                            <a className="text-[var(--text-orange-color)] lg:text-xl  text-sm">{"$ 18.00"}</a>
                        </div>
    ...
    return (
        <div className="lg:flex  flex-wrap  align-items-center  lg:pt-12 lg:pb-12">
            {listItems}
        </div>
    );
